...
function buildPostBlock(item: TaskItem): string | null {
  ...
  const imgLine = results
    // For webp thumbnails we use the directUrl to preserve aniamtion as thumbnails generated by the host usualy are static
    .map(
      (result) =>
        `[url=${result.pageUrl}][img]${(item.outputName?.endsWith(".webp") ? result.directUrl : result.mediumUrl) ?? result.thumbUrl}[/img][/url]`,
    )
    .join(" ");
