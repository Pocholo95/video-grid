...
describe("GridRenderer - video_with_audio sequence mode", () => {
  ...
  describe("FFmpeg per-segment cut commands", () => {
    ...
    it("uses -fps_mode cfr and -shortest for Firefox concat compatibility", async () => {
      const file = createTestSource();
      const meta = createTestMeta({ duration: 120 });
      ...
      // First 2 calls are segment cuts - should have -fps_mode cfr and -shortest
      for (let i = 0; i < 2; i++) {
        expect(execCalls[i]).toContain("-fps_mode");
